...
// HTTP API calls replacing Wails bindings (task 6.3)
import { providerApi, proxyApi, healthApi } from '@/api'
import { wsClient, WS_EVENTS } from '@/api'

async function LoadProviders(kind: string): Promise<any[]> {
  const res = await providerApi.load(kind)
  return res.data || []
}
async function SaveProviders(kind: string, providers: any[]): Promise<void> {
  await providerApi.save(kind, providers)
}
async function DuplicateProvider(kind: string, id: number): Promise<any> {
  const res = await providerApi.duplicate(kind, id)
  return res.data
}
async function RenameProvider(kind: string, id: number, newName: string): Promise<void> {
  await providerApi.rename(kind, id, newName)
}
async function GetGeminiProviders(): Promise<any[]> {
  const res = await providerApi.load('gemini')
  return res.data || []
}
async function UpdateGeminiProvider(provider: any): Promise<void> {
  const providers = await GetGeminiProviders()
  const updated = providers.map(p => p.id === provider.id ? provider : p)
  await providerApi.save('gemini', updated)
}
async function AddGeminiProvider(provider: any): Promise<any> {
  const providers = await GetGeminiProviders()
  providers.push(provider)
  await providerApi.save('gemini', providers)
  return provider
}
async function DeleteGeminiProvider(id: string): Promise<void> {
  const providers = await GetGeminiProviders()
  const filtered = providers.filter(p => p.id !== id)
  await providerApi.save('gemini', filtered)
}
async function ReorderGeminiProviders(ids: string[]): Promise<void> {
  // Convert string IDs to numbers for the API
  const numericIds = ids.map(id => typeof id === 'string' ? parseInt(id, 10) : id)
  await providerApi.reorder('gemini', numericIds)
}
...
const refreshDirectAppliedStatus = async (tab: ProviderTab = activeTab.value) => {
  ...
  try {
    ...
    if (tab === 'claude') {
      // TODO: Replace with HTTP API call (task 6.3)
      id = null
    } else if (tab === 'codex') {
      // TODO: Replace with HTTP API call (task 6.3)
      id = null
...
const handleDirectApply = async (card: AutomationCard) => {
  ...
  try {
    if (tab === 'claude') {
      // TODO: Replace with HTTP API call (task 6.3)
      console.warn('[stub] ApplySingleProvider claude not yet implemented')
    } else if (tab === 'codex') {
      // TODO: Replace with HTTP API call (task 6.3)
      console.warn('[stub] ApplySingleProvider codex not yet implemented')
    } else if (tab === 'gemini') {
      // TODO: Replace with HTTP API call (task 6.3)
      const index = cards.gemini.findIndex(c => c.id === card.id)
      if (index === -1 || !geminiProvidersCache.value[index]) return
      console.warn('[stub] ApplySingleProvider gemini not yet implemented')
    }
...
const handleResetLevel = async (providerName: string) => {
  try {
    // TODO: Replace with HTTP API call (task 6.3)
    console.warn('[stub] ManualResetLevel not yet implemented')
...
const loadLastUsedProviders = async () => {
  try {
    const res = await proxyApi.getLastUsed()
    const data = res.data
    if (data) {
      // 支持数组或单个对象
      if (Array.isArray(data)) {
        data.forEach((item: any) => {
          if (item.platform && item.provider_name) {
            lastUsedProviders[item.platform] = item
          }
        })
      } else if (data.platform && data.provider_name) {
        lastUsedProviders[data.platform] = data
      }
    }
  } catch (err) {
...
onMounted(async () => {
  ...
  // WebSocket event listeners (task 6.3)
  wsClient.connect()
  unsubscribeSwitched = wsClient.on('provider:switched', handleProviderSwitched)
  unsubscribeBlacklisted = wsClient.on('provider:blacklisted', handleProviderBlacklisted)
})
...
const handleTestConnectivity = async () => {
  ...
  try {
    ...
    // Use health API to check connectivity
    const editingId = Number(modalState.editingId)
    if (!editingId) {
      throw new Error('No provider selected')
    }
    
    const res = await healthApi.checkSingle(platform, editingId)
    const result = res.data
    
    connectivityTestResult.value = {
      success: result.success ?? false,
      message: result.success
        ? t('components.main.form.connectivity.success', { latency: result.latency ?? 0 })
        : result.error || t('components.main.form.connectivity.failed')
    }
  } catch (error) {
    connectivityTestResult.value = {
      success: false,
...
const handleDuplicate = async (card: AutomationCard) => {
  try {
    ...
    if (tab === 'gemini') {
      ...
      // TODO: Replace with HTTP API call (task 6.3)
      console.warn('[stub] GeminiService.DuplicateProvider not yet implemented')
      const newProvider = null
