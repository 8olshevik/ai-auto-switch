...
async function GetPrompts(platform: string): Promise<Prompt[]> {
  const res = await promptsApi.list(platform)
  return res.data || []
}

async function UpsertPrompt(platform: string, id: string | number, prompt: Prompt): Promise<Prompt> {
  const promptData = { ...prompt, id }
  if (prompt.id) {
    await promptsApi.update(platform, String(prompt.id), prompt)
  } else {
    await promptsApi.create(platform, id as string, prompt)
  }
  return prompt
}

async function DeletePrompt(platform: string, id: string | number): Promise<void> {
  await promptsApi.delete(platform, String(id))
}

async function EnablePrompt(platform: string, id: string | number): Promise<void> {
  // Get the prompt and update enabled status
  const prompts = await GetPrompts(platform)
  const prompt = prompts.find(p => String(p.id) === String(id))
  if (prompt) {
    await UpsertPrompt(platform, id, { ...prompt, enabled: true })
  }
}

async function ImportFromFile(platform: string, _filePath: string): Promise<Prompt> {
  // For web, we need a file upload dialog
  return { name: '', content: '', platform, enabled: true }
}

async function GetCurrentFileContent(platform: string): Promise<string> {
...
async function loadPrompts() {
  loading.value = true
  try {
    const list = await GetPrompts(activePlatform.value)
    prompts.value = {}
    list.forEach(p => {
      if (p.id) prompts.value[p.id] = p
    })
