...
// HTTP API calls (task 6.3)
import { providerApi } from '@/api'
import { proxyApi } from '@/api'

async function GetPresets(): Promise<any[]> {
  // Presets could be fetched from backend or hardcoded for now
  return []
}
...
async function GetStatus(): Promise<any> {
  ...
  return {
    ...
    enabled: res.data.running || false
  }
}

async function AddProvider(provider: any): Promise<any> {
  const providers = await GetProviders()
  const newProvider = { ...provider, id: provider.id || Date.now() }
  providers.push(newProvider)
  await providerApi.save('gemini', providers)
  return newProvider
}

async function UpdateProvider(provider: any): Promise<void> {
  const providers = await GetProviders()
...
async function DeleteProvider(id: string): Promise<void> {
  const providers = await GetProviders()
  const filtered = providers.filter(p => String(p.id) !== id)
  await providerApi.save('gemini', filtered)
}

async function SwitchProvider(id: string): Promise<void> {
  // Switch by enabling this provider and disabling others
  const providers = await GetProviders()
  const updated = providers.map(p => ({
    ...p,
    enabled: String(p.id) === id
  }))
  await providerApi.save('gemini', updated)
}

async function CreateProviderFromPreset(presetId: string): Promise<any> {
  // For presets, we'd need the preset data
  return null
}
...
type BindingGeminiStatus = any
type BindingGeminiProvider = any
type BindingGeminiPreset = any
type GeminiAuth = string
