...
type ProviderRelayService struct {
	...
	wsHub               WSBroadcaster                // WebSocket Hub for broadcasting events
}
...
// SetWSHub sets the WebSocket broadcaster for real-time event pushing.
func (prs *ProviderRelayService) SetWSHub(hub WSBroadcaster) {
	prs.wsHub = hub
}
...
func (prs *ProviderRelayService) Start() error {
	...
	router := gin.Default()
	// 添加 API Key 认证中间件（用于第三方应用访问代理服务）
	router.Use(middleware.ProxyAPIKeyAuthMiddleware())
	// 保留速率限制中间件
	...
	// Broadcast proxy status change
	if prs.wsHub != nil {
		prs.wsHub.BroadcastEvent(WSEventProxyStatus, map[string]interface{}{
			"running": true,
			"addr":    prs.addr,
		})
	}
...
func (prs *ProviderRelayService) forwardRequest(
	...
	defer func() {
		...
		// Broadcast request log event via WebSocket
		if prs.wsHub != nil {
			prs.wsHub.BroadcastEvent(WSEventRequestLog, requestLog)
		}
	}()
