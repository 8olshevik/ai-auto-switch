...
type HealthCheckService struct {
	...
	// WebSocket Hub for broadcasting health check results
	wsHub WSBroadcaster
}
...
// SetWSHub sets the WebSocket broadcaster for real-time health check result pushing.
func (hcs *HealthCheckService) SetWSHub(hub WSBroadcaster) {
	hcs.wsHub = hub
}
...
func (hcs *HealthCheckService) RunSingleCheck(platform string, providerID int64) (*HealthCheckResult, error) {
	...
	// Broadcast health check result via WebSocket
	if hcs.wsHub != nil {
		hcs.wsHub.BroadcastEvent(WSEventHealthCheck, result)
	}
