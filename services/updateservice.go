...
// 常量
const (
	latestJSONURL     = "https://github.com/Rogers-F/ai-auto-switch/releases/latest/download/latest.json"
	githubAPIURL      = "https://api.github.com/repos/Rogers-F/ai-auto-switch/releases/latest"
...
var allowedURLPrefixes = []string{
	"https://github.com/Rogers-F/ai-auto-switch/releases/download/",
	"https://github.com/Rogers-F/ai-auto-switch/releases/latest/download/",
...
// Note: Update events are handled via WebSocket in the web server mode
...
func (us *UpdateService) RequestRestart() error {
	...
	// P0: 退出应用，让更新脚本接管
	// Note: In web server mode, update restart is handled differently
	log.Printf("[Update] Update ready to apply, version: %s", targetInfo.Version)
...
// emitState 发送状态事件 - handled via WebSocket in web server mode
func (us *UpdateService) emitState() {
	log.Printf("[Update] State: %s", us.state)
}

// emitStateUnlocked 发送状态事件 - handled via WebSocket in web server mode
...
// emitProgressThrottled 节流发送进度事件 - handled via WebSocket in web server mode
