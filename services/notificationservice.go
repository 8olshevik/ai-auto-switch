...
type NotificationService struct {
	...
	wsHub          interface{}      // WebSocket Hub，用于广播事件
	mu             sync.RWMutex
...
// SetWSHub 设置 WebSocket Hub（用于发送事件到前端）
func (ns *NotificationService) SetWSHub(hub interface{}) {
	ns.wsHub = hub
}
...
// emitSwitchEvent 发送切换事件到前端 via WebSocket
func (ns *NotificationService) emitSwitchEvent(info SwitchNotification) {
	// WebSocket mode handled in cmd/server via SetWSHub
	// Desktop app uses native notifications
	log.Printf("[Notification] Provider switched: %s → %s", info.FromProvider, info.ToProvider)
}
...
// emitBlacklistEvent 发送拉黑事件到前端 via WebSocket
func (ns *NotificationService) emitBlacklistEvent(platform, providerName string, level, durationMinutes int) {
	// WebSocket mode handled in cmd/server via SetWSHub
	// Desktop app uses native notifications
	log.Printf("[Notification] Provider blacklisted: %s (L%d, %d min)", providerName, level, durationMinutes)
}
